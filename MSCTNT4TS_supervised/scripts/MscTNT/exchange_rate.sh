if [ ! -d "./logs" ]; then
    mkdir ./logs
fi

if [ ! -d "./logs/LongForecasting" ]; then
    mkdir ./logs/LongForecasting
fi
seq_len=810
model_name=MscTNT4TS

root_path_name=./dataset/
data_path_name=exchange_rate.csv
model_id_name=EXCHANGE_shuff_5rand_re3_patch90
data_name=custom

# random_seed=2021
# for random_seed in 2021 2022 2023 2024 2025
for random_seed in 2021
do

# for shuffle in "None" "RandShuf" "HalfEx"
for shuffle in "None"
do
# for patch_len in 18 36 54 72 90 看長patchlen是否抵抗shuff
# do

# for pred_len in 96 192 336 720 
for pred_len in 96 
do
    python -u run_longExp.py \
      --random_seed $random_seed \
      --is_training 1 \
      --root_path $root_path_name \
      --data_path $data_path_name \
      --model_id $model_id_name"_"$seq_len'_'$pred_len \
      --model $model_name \
      --data $data_name \
      --features M \
      --seq_len $seq_len \
      --pred_len $pred_len \
      --enc_in 8 \
      --e_layers 1 \
      --n_heads 4 \
      --outer_n_heads 4\
      --inner_n_heads 4\
      --outer_dim 16\
      --inner_dim 8\
      --outer_mlp_ratio 10\
      --inner_mlp_ratio 10\
      --pos_drop 0\
      --head_dropout 0.15\
      --inner_tcn_drop 0.15 \
      --outer_tcn_drop 0.15 \
      --inner_attn_dropout 0.15 \
      --outer_attn_dropout 0.15 \
      --inner_proj_dropout 0.25 \
      --outer_proj_dropout 0.25 \
      --patch_len 90\
      --patch_stride 90\
      --subpatch_len 9\
      --subpatch_stride 9\
      --des 'Exp' \
      --train_epochs 100\
      --patience 30\
      --itr 1 --batch_size 128 --learning_rate 0.00006\
      --pct_start 0.2\
      --lradj 'TST'\
      --revin 0\
      --subtract_last 1\
      --affine 0\
      --early_inner_tcn_layers 3\
      --early_outer_tcn_layers 3\
      --inner_tcn_layers 0\
      --outer_tcn_layers 0\
      --final_div_factor 10000\
      --shuffle_method $shuffle \
      | tee -a logs/LongForecasting/$model_name'_'$model_id_name'_'$seq_len'_'$pred_len.log
done
done
done